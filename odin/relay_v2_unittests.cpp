...
// loop), injecting readiness by calling the exported odin_relay_v2_ready
...
// deadline harness (replicated below as RelayV2RunDeadline) plus a per-row
// watchdog timer.
 ...
 public:
  ...
  static void Run(Fn fn) {
    ...
    if (!exited) {
      ...
      FAIL() << "RelayV2RunDeadline exceeded 2 seconds";
    }
...
struct DoneState {
  int calls = 0;
  odin_relay_v2_status_t status = ODIN_RELAY_V2_OK;
  int err = 0;
...
void OnDone(odin_relay_v2_t *relay, odin_relay_v2_status_t status, int err,
            void *user_data) {
...
// calling odin_relay_v2_ready(&fake.base, events, r) directly.
...
TEST(OdinRelayV2Test, T1) {
  ...
  DoneState state;
  odin_relay_v2_t *r = nullptr;
  ASSERT_EQ(odin_relay_v2_create(OnDone, &state, &r), 0) << std::strerror(errno);
  ASSERT_EQ(odin_relay_v2_start(r, &a.base, &b.base), 0) << std::strerror(errno);

  odin_relay_v2_ready(&a.base, ODIN_TRANSPORT_READ, r);
  odin_relay_v2_ready(&b.base, ODIN_TRANSPORT_READ, r);
  odin_relay_v2_ready(&b.base, ODIN_TRANSPORT_WRITE, r);
  odin_relay_v2_ready(&a.base, ODIN_TRANSPORT_WRITE, r);
...
TEST(OdinRelayV2Test, T3) {
  ...
  odin_relay_v2_ready(&a.base, ODIN_TRANSPORT_READ, r);  // read "abc"
  odin_relay_v2_ready(&b.base, ODIN_TRANSPORT_READ, r);  // read "xyz"
  odin_relay_v2_ready(&b.base, ODIN_TRANSPORT_WRITE, r); // flush "abc" -> b
  odin_relay_v2_ready(&a.base, ODIN_TRANSPORT_WRITE, r); // flush "xyz" -> a
  odin_relay_v2_ready(&a.base, ODIN_TRANSPORT_READ, r);  // a EOF -> shutdown b
  odin_relay_v2_ready(&b.base, ODIN_TRANSPORT_READ, r);  // b EOF -> shutdown a
  ...
  EXPECT_EQ(state.status, ODIN_RELAY_V2_OK);
...
TEST(OdinRelayV2Test, T4) {
  ...
  odin_relay_v2_ready(&a.base, ODIN_TRANSPORT_READ, r);  // read "z" into dir A
  odin_relay_v2_ready(&b.base, ODIN_TRANSPORT_WRITE, r); // write -> b fails EPIPE

  EXPECT_EQ(state.calls, 1);
  EXPECT_EQ(state.status, ODIN_RELAY_V2_ERROR);
...
TEST(OdinRelayV2Test, T6) {
  // (a) An ERROR readiness with no synchronous failure probes odin_transport_error.
  {
    ...
    DoneState state;
    odin_relay_v2_t *r = nullptr;
    ASSERT_EQ(odin_relay_v2_create(OnDone, &state, &r), 0)
        << std::strerror(errno);
    ASSERT_EQ(odin_relay_v2_start(r, &a.base, &b.base), 0)
        << std::strerror(errno);

    odin_relay_v2_ready(&a.base, ODIN_TRANSPORT_ERROR, r);
  ...
  // (b) A benign ERROR alongside a progressing read latches no error.
  {
    ...
    odin_relay_v2_ready(&a.base, ODIN_TRANSPORT_READ | ODIN_TRANSPORT_ERROR, r);
    EXPECT_EQ(state.calls, 0); // outcome stays open

    odin_relay_v2_ready(&b.base, ODIN_TRANSPORT_WRITE, r); // flush "d" -> b
...
TEST(OdinRelayV2Test, T8) {
  RelayV2RunDeadline::Run([] {
    int fd_a = -1;
    int pa = -1;
    int fd_b = -1;
    int pb = -1;
    ...
    ASSERT_EQ(odin_fd_transport_create(loop, fd_a, odin_relay_v2_ready, r, &a),
              0)
        << std::strerror(errno);
    ASSERT_EQ(odin_fd_transport_create(loop, fd_b, odin_relay_v2_ready, r, &b),
              0)
        << std::strerror(errno);
    ...
    ASSERT_EQ(odin_event_timer_start(loop, 100000, 0, WatchdogCb, &state,
                                     &watchdog),
              0);
    ASSERT_EQ(odin_relay_v2_start(r, a, b), 0) << std::strerror(errno);
...
TEST(OdinRelayV2Test, T12) {
  RelayV2RunDeadline::Run([] {
    ...
    errno = 0;
    EXPECT_EQ(odin_relay_v2_start(r, a, b), -1);
...
void T15TimerCb(odin_event_loop_t *loop, odin_event_timer_t *timer,
  ...
  s->x_ok = (n == 1 && buf[0] == 'x');
  odin_relay_v2_destroy(s->relay); // abort the still-running relay
  (void)write(s->pa, "y", 1);
  (void)write(s->pb, "z", 1);
}
} // namespace
...
TEST(OdinRelayV2Test, T15) {
  RelayV2RunDeadline::Run([] {
    ...
    odin_relay_v2_t *r = nullptr;
    ASSERT_EQ(odin_relay_v2_create(OnDone, &s.done, &r), 0)
        << std::strerror(errno);
    odin_transport_t *a = nullptr;
    odin_transport_t *b = nullptr;
    ASSERT_EQ(odin_fd_transport_create(loop, fd_a, odin_relay_v2_ready, r, &a),
