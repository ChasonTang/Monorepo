...
odin_proto_status_t odin_proto_encode_connect_req(const char *host,
  ...
  const size_t encoded_size = 5 + host_len;
  if (cap < encoded_size) {
    return ODIN_PROTO_ERR_BUF_TOO_SMALL;
  }

  buf[0] = ODIN_PROTO_VERSION_V1;
  buf[1] = ODIN_PROTO_FRAME_CONNECT_REQ;
  buf[2] = (uint8_t)host_len;
  memcpy(&buf[3], host, host_len);
  buf[3 + host_len] = (uint8_t)((port >> 8) & 0xFFu);
  buf[4 + host_len] = (uint8_t)(port & 0xFFu);
  *out_n = encoded_size;
...
odin_proto_status_t odin_proto_decode_connect_req(
  ...
  if (host_cap < (size_t)host_len + 1) {
    return ODIN_PROTO_ERR_BUF_TOO_SMALL;
  }

  memcpy(host_out, &buf[3], (size_t)host_len);
  host_out[host_len] = '\0';
  *out_host_len = host_len;
  *out_port = (uint16_t)(((uint16_t)buf[3 + host_len] << 8) |
...
odin_proto_status_t odin_proto_encode_connect_resp(uint16_t error_code,
  ...
  if (cap < ODIN_PROTO_CONNECT_RESP_SIZE) {
    return ODIN_PROTO_ERR_BUF_TOO_SMALL;
  }

  buf[0] = ODIN_PROTO_VERSION_V1;
  buf[1] = ODIN_PROTO_FRAME_CONNECT_RESP;
  buf[2] = (uint8_t)((error_code >> 8) & 0xFFu);
  buf[3] = (uint8_t)(error_code & 0xFFu);
  *out_n = ODIN_PROTO_CONNECT_RESP_SIZE;
  return ODIN_PROTO_OK;
}
...
odin_proto_status_t odin_proto_encode_connect_req_v2(
    ...
    odin_proto_iov_t out_iov[3], uint8_t scratch_header[3],
