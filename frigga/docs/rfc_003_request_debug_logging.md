...
If the request stream emits an `error` event before `end`, the collected partial body is discarded and `handleRequest` is not called. Instead, a `request_error` log entry is emitted to stderr with the available request metadata:

```json
{"timestamp":"2026-03-16T10:15:32.100Z","level":"ERROR","event":"request_error","method":"POST","url":"/v1/messages","request_headers":{"content-type":"application/json"},"message":"ECONNRESET"}
```
...
| `request_headers` | object | All request headers available at the time of the error |
| `message` | string | Error message from the `error` event (`err.message`) |
...
| `stderr` | WARN, ERROR | `shutdown` (timeout), `server_error`, `request_error` |
...
| 8 | Request stream error emits request_error log | Client disconnects mid-body | stderr line parses as JSON; contains `event: "request_error"`, `method`, `url`, `request_headers`, `message`; `handleRequest` not called; no `event: "request"` log emitted |
...
- [x] Update existing integration test log assertions (startup log format, shutdown log format) to expect NDJSON
- [x] Add integration tests for request log NDJSON fields: `request_headers`, `request_body`, `response_headers`, `response_body`
- [x] Add integration test for empty request body case
- [x] Add integration test for request stream error (`request_error` log emission)
...
- [x] Add request body collection (`data`/`end` events) in the `http.createServer` callback in `startServer`
- [x] Replace all `process.stdout.write` / `process.stderr.write` log calls in `server.js` with `JSON.stringify()` + `\n` NDJSON output
...
|---------|------|--------|---------|
| 1.2 | 2026-03-16 | Chason Tang | Add `request_error` log event for request stream errors during body collection; add test scenario 8; remove RFC-001 update task from implementation plan |
