...
The existing `source_set("odin")` gains `odin/cli.{c,h}` beside `protocol.{c,h}`. The existing `executable("protocol_unittests")` is renamed to `executable("odin_unittests")`, adds `odin/cli_unittests.cpp`, and lists `:odin_cli_artifacts` in `data_deps` so a clean `ninja -C out/ tests` (without prior `default`) produces both the test binary and the runtime artifacts T8/T9/T10 spawn (`out/odin`, `odin-client`, `odin-server`) — one library source_set and one test binary cover `odin/`. A new `executable("odin_main")` over `odin/main.c` uses `deps = [ ":odin" ]` and `output_name = "odin"`; the target is named `odin_main` because `executable("odin")` would collide with `source_set("odin")` in one BUILD.gn. `.gn` gains `script_executable = "python3"` so GN actions run through the repo's supported Python runner. A new `action("odin_symlinks")` runs `build/symlink.py` (added in P1) to create `$root_out_dir/odin-client` and `$root_out_dir/odin-server` as relative symlinks to basename `odin`; the action intentionally has no `deps` on `:odin_main`, because the symlink contents depend only on the link target basename and link paths, not on the current `out/odin` binary timestamp. A new `group("odin_cli_artifacts")` depends on both `:odin_main` and `:odin_symlinks`, giving callers one target for the runnable CLI artifacts without making every `odin` relink re-run the symlink action. The root `BUILD.gn` `default` group gains `"//odin:odin_cli_artifacts"` and the `tests` group entry changes from `"//odin:protocol_unittests"` to `"//odin:odin_unittests"`. The binary's only transitive dep is `:odin` (library code, no transport), so its link surface stays libc-only. The skeleton has no I/O beyond the success banner, usage, and `exit`.
