 ...
 *
 * Hand-tailored ares_config.h for the GN build on macOS (clang, 10.12+).
 * Mirrors what `cmake -DCARES_THREADS=ON` would generate for Darwin so the
 * GN build does not depend on the autotools/cmake configure step.
 */
...
/* Argument types for getnameinfo(3) on macOS. */
...
/* time.h + sys/time.h coexist on macOS. */
