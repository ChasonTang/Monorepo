...
- **Don't expand scope.** No refactors, helpers, comments, file moves, or features beyond what §2 Goals, §3.2 contract surfaces, §4 compatibility entries, §5 security entries, and §7 phase scopes pin. The reviewers approved this scope; growing it now reopens review.
- **Don't shrink scope.** Every `G#` must be implemented; every `T#` must transition red→green across §7 phases; every `B#` migration and paired §6 regression row must be complete no later than the phase that introduces the changed documented contract; every `S#` security mitigation and paired §6 enforcement row must be complete no later than the phase that exposes the trigger surface.
...
3. For `T#` rows this phase introduces **red**, run the red-verification command or mode named in `Scope`/`Done when` and confirm the row assertions fail against the current stub/behavior. Then apply or verify the named gate (`strict xfail` / `skip` / feature flag / separate test target) and confirm the project's default local test suite stays green. `skip`/flag-gate/separate-target alone do not prove red because they do not execute the assertions; `strict xfail` proves red only when the framework reports the expected failure rather than XPASS.
...
2. Verify the final working tree still satisfies every `G#`, every §3.2 contract surface, every `B#` migration, and every `S#` security mitigation.
...
3. **RFC coverage:** every `G#`, §3.2 contract surface, `B#` migration, and `S#` security mitigation verified against the staged diff.
4. **Deviations from RFC:** list any, each with — where the RFC said X, what you did instead, why (cite the user's clarifying answer; never invent justification). `None` is the expected default.
