...
7. **Hard-cap preservation.** After applying a revision, run `python3 docs/rfc_lint.py <rfc-path>`. If the revised RFC violates a single-RFC hard cap, do not continue patching the same document unless the user explicitly approved an exception and the RFC contains an `Exception:` paragraph before §1 naming the exceeded cap. Otherwise, change the revision response to a split proposal: which smaller RFCs should replace this one and which current findings each split owns. Do not add subcases to an already oversized `T#` row to avoid the `T#` row-count cap.
