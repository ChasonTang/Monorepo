...
- **G1.** The client CLI parser requires a non-empty `--ca-file FILE`, stores the selected path for Client mode, rejects omission as a missing required flag, and rejects malformed CA-file option forms without changing Server-mode parsing.
- **G2.** A successful `odin-client --ca-file FILE` startup passes that exact path into the client QUIC runtime default configuration after the local listener is ready, while a startup that omits `--ca-file` fails before invoking the client runner or creating a QUIC runtime.
...
Omitting `--ca-file` is a Client-mode CLI error. The parser records no CA path and reports `ODIN_CLI_ERR_MISSING_REQUIRED`; the client runner is not invoked and no QUIC runtime is created. Supplying `--ca-file` makes the CA store runtime-owned before any UDP driver or QUIC connection is started. The lower-level default runtime still preserves its `ca_file == NULL` behavior for direct API callers and focused runtime tests, but the `odin-client` entry point no longer uses that unverified path.
...
**Unstated contract.** `quic_ca_file` is initialized to `NULL` before parsing. `--ca-file FILE` and `--ca-file=FILE` are Client-mode only, alias the option value provided by `getopt_long` without allocation or canonicalization, and are accepted only when `FILE[0] != '\0'`. With otherwise-valid Client `--listen` and `--server`, omitting `--ca-file` returns `ODIN_CLI_ERR_MISSING_REQUIRED` with `mode == ODIN_CLI_MODE_CLIENT`, leaves `quic_ca_file == NULL`, and maps through `odin_cli_main` to `odin: missing required flag\n<U_C>\n` using the updated Client usage. With otherwise-valid Client required options, an empty value from either `--ca-file ""` or `--ca-file=` returns `ODIN_CLI_ERR_BAD_QUIC_TLS` with `mode == ODIN_CLI_MODE_CLIENT`, leaves `quic_ca_file == NULL`, and maps through `odin_cli_main` to `odin: invalid QUIC TLS configuration\n<U_C>\n`. If the same argv also has a bad `--listen` value, bad `--server` value, or omitted required Client option, the existing `ODIN_CLI_ERR_BAD_LISTEN_PORT`, `ODIN_CLI_ERR_BAD_SERVER`, or `ODIN_CLI_ERR_MISSING_REQUIRED` result wins before `ODIN_CLI_ERR_BAD_QUIC_TLS`, and `odin_cli_main` emits the corresponding existing Client banner with the updated `usage: odin-client --listen ADDR --server ADDR --ca-file FILE\n` line. A missing required argument, abbreviated long spelling such as `--ca`, stray positional value, or Server-mode `--ca-file` remains `ODIN_CLI_ERR_UNKNOWN_FLAG` under the existing precedence. Help and unknown-flag precedence do not change. Server-mode `--quic-cert` and `--quic-key` behavior remains unchanged, and Server mode never writes `quic_ca_file`. `odin_cli_run_client` also rejects `quic_ca_file == NULL` or an empty CA path as a config error, so direct runner calls cannot create the default QUIC client runtime without a CA file.
...
parse(argc, argv, out):
  ...
  preserve existing status precedence:
    ...
    if bad listen, bad server, missing --server, or missing --ca-file:
      return the existing error
  if bad_quic_tls:
    return ODIN_CLI_ERR_BAD_QUIC_TLS
  on Client OK:
    out.quic_ca_file = ca_file_arg
...
Satisfies: G1 via the exact required Client option, empty-value rejection, omitted-option failure, and Server-mode isolation; G2 via the client-runner config field and runner-side non-null CA guard.
...
| State | CLI omitted CA file rejected | T1, T2, T3 |
...
|---|----------|---------------|-----------------|--------|-------|
| T1 | Client parser requires and accepts CA file | Subcases: `odin-client --listen 8080 --server 127.0.0.1:4433`, `odin-client --listen 8080 --server 127.0.0.1:4433 --ca-file <repo>/thor/out/root-ca.pem`, `odin-client --listen 8080 --server 127.0.0.1:4433 --ca-file=<repo>/thor/out/root-ca.pem` | Omitted case returns `ODIN_CLI_ERR_MISSING_REQUIRED` and has `quic_ca_file == NULL`; separate-argument supplied case returns `ODIN_CLI_OK` and has `quic_ca_file` pointer equal to the CA argv slot; equals-form supplied case returns `ODIN_CLI_OK` and stores the exact path string from after `--ca-file=`; listen, server host, server port, and `client_transport` match the existing Client OK contract for supplied CA cases | G1 | unit |
| T2 | Bad CA-file option forms and precedence are pinned | Common valid Client prefix is `odin-client --listen 8080 --server 127.0.0.1:4433`. Subcase groups: omitted CA with valid required options (`<prefix>`); empty CA with valid required options (`<prefix> --ca-file ""`, `<prefix> --ca-file=`); bad listen plus empty CA (`odin-client --listen abc --server 127.0.0.1:4433 --ca-file ""`); bad server plus empty CA (`odin-client --listen 8080 --server 127.0.0.1:bad --ca-file ""`); missing required plus empty CA (`odin-client --listen 8080 --ca-file ""`); invalid option forms (`<prefix> --ca-file` with no argument, `<prefix> --ca` abbreviation, `<prefix> --ca-file <repo>/thor/out/root-ca.pem extra`, and `odin-server --ca-file <repo>/thor/out/root-ca.pem --listen 4433 --quic-cert C --quic-key K`); help precedence (`odin-client --help --ca-file ""`) | Omitted CA with otherwise valid required options returns `ODIN_CLI_ERR_MISSING_REQUIRED`; empty CA with valid required options returns `ODIN_CLI_ERR_BAD_QUIC_TLS`, Client mode, and `quic_ca_file == NULL`; `odin_cli_main` writes `odin: invalid QUIC TLS configuration\nusage: odin-client --listen ADDR --server ADDR --ca-file FILE\n` for empty CA and `odin: missing required flag\nusage: odin-client --listen ADDR --server ADDR --ca-file FILE\n` for omitted CA, returning 2 without invoking the runner. Bad listen plus empty CA returns `ODIN_CLI_ERR_BAD_LISTEN_PORT`; bad server plus empty CA returns `ODIN_CLI_ERR_BAD_SERVER`; missing server plus empty CA returns `ODIN_CLI_ERR_MISSING_REQUIRED`. All precedence cases leave `quic_ca_file == NULL`. Missing argument, abbreviation, stray positional, and Server-mode use return `ODIN_CLI_ERR_UNKNOWN_FLAG`; Server mode leaves `mode == ODIN_CLI_MODE_SERVER` and `quic_ca_file == NULL`; help returns `ODIN_CLI_HELP` and prints only Client usage to stdout | G1, S2 | unit |
| T3 | CLI runner forwards required CA file and rejects omission | One successful `odin_cli_main` child with `--listen 0 --server 127.0.0.1:4433 --ca-file <repo>/thor/out/root-ca.pem`, plus one omitted-CA run with `--listen 0 --server 127.0.0.1:4433`. Parent deadline-reads the ready line and runtime snapshot pipe for the successful child, sends `SIGTERM`, and waits with the existing 2 s child deadline | Supplied child ready line remains `odin: mode=client transport=quic listen=<port> server=127.0.0.1:4433\n`; it records the repo-root CA path in the CLI runtime-config and default-create records, `conn_ssl_config_value.cert_verify_flag == XQC_TLS_CERT_FLAG_NEED_VERIFY`, non-null `transport_callbacks_value.cert_verify_cb`, one CA-store load, and exactly one CA-store free during CLI `force_destroy` cleanup. Omitted run returns 2 with the missing-required usage banner, records no default-create call, and leaves listener, accept loop, runtime, signal timer, and event-loop liveness counters at zero | G2 | integration |
