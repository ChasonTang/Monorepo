/* odin/relay_v2.c -- RFC-014 transport-agnostic bidirectional byte relay.
 *
 ...
 * transport's dependencies. It keeps the RFC-011 relay's fixed 64 KiB
...
#define ODIN_RELAY_V2_CAP 65536u

/* Direction indices: A = a -> b, B = b -> a. */
#define ODIN_RELAY_V2_DIR_A 0
#define ODIN_RELAY_V2_DIR_B 1

/* Relay-wide outcome: set once by the first fault (or dual end-of-stream),
 * then consumed once by teardown. */
enum {
  ODIN_RELAY_V2_OUTCOME_NONE = 0,
  ODIN_RELAY_V2_OUTCOME_OK,
  ODIN_RELAY_V2_OUTCOME_ERROR,
};

/* do_read result classification. */
enum {
  ODIN_RELAY_V2_READ_AGAIN = 0,
  ODIN_RELAY_V2_READ_PROGRESS,
  ODIN_RELAY_V2_READ_EOF,
  ODIN_RELAY_V2_READ_FAILED,
};
...
typedef struct {
  odin_transport_t *t;
  odin_relay_v2_dir_t *src;
  odin_relay_v2_dir_t *sink;
  unsigned int cur;
} odin_relay_v2_end_t;
...
struct odin_relay_v2_t {
  ...
  void *user_data;
  odin_relay_v2_dir_t dir[2];
  odin_relay_v2_end_t end[2];
  int outcome;
  int err;
  int torn_down;
};
...
static int do_read(odin_relay_v2_t *r, odin_relay_v2_dir_t *d) {
  const size_t tail = (d->head + d->len) % ODIN_RELAY_V2_CAP;
  size_t run = ODIN_RELAY_V2_CAP - d->len;
  if (run > ODIN_RELAY_V2_CAP - tail) {
    run = ODIN_RELAY_V2_CAP - tail;
  }
  size_t n = 0;
  switch (odin_transport_read(d->src_t, d->buf + tail, run, &n)) {
    case ODIN_TRANSPORT_OK:
      d->len += n;
      return ODIN_RELAY_V2_READ_PROGRESS;
    case ODIN_TRANSPORT_EOF:
      d->read_eof = 1;
      return ODIN_RELAY_V2_READ_EOF;
    case ODIN_TRANSPORT_AGAIN:
      return ODIN_RELAY_V2_READ_AGAIN;
    case ODIN_TRANSPORT_IO_ERROR:
      break;
  }
  r->outcome = ODIN_RELAY_V2_OUTCOME_ERROR;
  r->err = errno; /* errno set by transport (RFC-013) */
  return ODIN_RELAY_V2_READ_FAILED;
}
...
static void do_write(odin_relay_v2_t *r, odin_relay_v2_dir_t *d) {
  size_t run = d->len;
  if (run > ODIN_RELAY_V2_CAP - d->head) {
    run = ODIN_RELAY_V2_CAP - d->head;
  }
  size_t n = 0;
  switch (odin_transport_write(d->sink_t, d->buf + d->head, run, &n)) {
    case ODIN_TRANSPORT_OK:
      d->head = (d->head + n) % ODIN_RELAY_V2_CAP;
      d->len -= n;
      return;
    case ODIN_TRANSPORT_AGAIN:
      return;
 ...
 * set_interest absorbs the lazy start/update/stop the RFC-011 reconcile did by
 * hand (odin/relay.c:149-177). */
static void reconcile(odin_relay_v2_t *r, odin_relay_v2_end_t *e) {
  unsigned int m = 0;
  if (e->src->read_eof == 0 && e->src->len < ODIN_RELAY_V2_CAP) {
 ...
 * half-closed (mirrors drive, odin/relay.c:205-223). */
static void drive(odin_relay_v2_t *r) {
  for (int i = 0; i < 2; ++i) {
    odin_relay_v2_dir_t *d = &r->dir[i];
  ...
  reconcile(r, &r->end[0]);
  reconcile(r, &r->end[1]);
  if (r->outcome == ODIN_RELAY_V2_OUTCOME_NONE &&
      r->dir[ODIN_RELAY_V2_DIR_A].write_shut &&
      r->dir[ODIN_RELAY_V2_DIR_B].write_shut) {
    r->outcome = ODIN_RELAY_V2_OUTCOME_OK;
  }
}
 ...
 * on_done returns, so odin_relay_v2_destroy from inside on_done is safe (mirrors
 * teardown, odin/relay.c:182-201). */
static void teardown(odin_relay_v2_t *r) {
  if (r->torn_down) {
    return;
  }
  r->torn_down = 1;
  ...
  const odin_relay_v2_done_cb cb = r->on_done;
  void *const ud = r->user_data;
  const odin_relay_v2_status_t st =
      (r->outcome == ODIN_RELAY_V2_OUTCOME_OK) ? ODIN_RELAY_V2_OK
                                               : ODIN_RELAY_V2_ERROR;
  const int e = (st == ODIN_RELAY_V2_OK) ? 0 : r->err;
  cb(r, st, e, ud);
}

int odin_relay_v2_create(odin_relay_v2_done_cb on_done, void *user_data,
                         odin_relay_v2_t **out) {
  odin_relay_v2_t *r = (odin_relay_v2_t *)calloc(1, sizeof(*r));
  if (r == NULL) {
    errno = ENOMEM;
    return -1;
  }
  r->dir[ODIN_RELAY_V2_DIR_A].buf =
      (unsigned char *)malloc(ODIN_RELAY_V2_CAP);
  r->dir[ODIN_RELAY_V2_DIR_B].buf =
      (unsigned char *)malloc(ODIN_RELAY_V2_CAP);
  if (r->dir[ODIN_RELAY_V2_DIR_A].buf == NULL ||
      r->dir[ODIN_RELAY_V2_DIR_B].buf == NULL) {
    free(r->dir[ODIN_RELAY_V2_DIR_A].buf);
    free(r->dir[ODIN_RELAY_V2_DIR_B].buf);
    free(r);
    errno = ENOMEM;
    return -1;
  }
  ...
  r->outcome = ODIN_RELAY_V2_OUTCOME_NONE;
 ...
 * endpoint that fired (mirrors on_ready, odin/relay.c:229-264). */
void odin_relay_v2_ready(odin_transport_t *t, unsigned int events,
                         void *user_data) {
  odin_relay_v2_t *r = (odin_relay_v2_t *)user_data;
  odin_relay_v2_end_t *e = (t == r->end[0].t) ? &r->end[0] : &r->end[1];
  odin_relay_v2_dir_t *src = e->src;
  odin_relay_v2_dir_t *sink = e->sink;
  const int err = (events & ODIN_TRANSPORT_ERROR) != 0;

  if (r->outcome == ODIN_RELAY_V2_OUTCOME_NONE && sink->len > 0 &&
      ((events & ODIN_TRANSPORT_WRITE) || err)) {
    do_write(r, sink);
  }

  int rd = ODIN_RELAY_V2_READ_AGAIN;
  if (r->outcome == ODIN_RELAY_V2_OUTCOME_NONE && src->read_eof == 0 &&
      src->len < ODIN_RELAY_V2_CAP && ((events & ODIN_TRANSPORT_READ) || err)) {
    rd = do_read(r, src);
  }

  if (r->outcome == ODIN_RELAY_V2_OUTCOME_NONE && err && src->read_eof == 0 &&
      rd != ODIN_RELAY_V2_READ_PROGRESS) {
...
int odin_relay_v2_start(odin_relay_v2_t *relay, odin_transport_t *a,
                        odin_transport_t *b) {
  /* dir A: a -> b; dir B: b -> a. */
  relay->dir[ODIN_RELAY_V2_DIR_A].src_t = a;
  relay->dir[ODIN_RELAY_V2_DIR_A].sink_t = b;
  relay->dir[ODIN_RELAY_V2_DIR_B].src_t = b;
  relay->dir[ODIN_RELAY_V2_DIR_B].sink_t = a;

  /* end[0] = a sources A, sinks B; end[1] = b sources B, sinks A. */
  relay->end[0].t = a;
  relay->end[0].src = &relay->dir[ODIN_RELAY_V2_DIR_A];
  relay->end[0].sink = &relay->dir[ODIN_RELAY_V2_DIR_B];
  relay->end[1].t = b;
  relay->end[1].src = &relay->dir[ODIN_RELAY_V2_DIR_B];
  relay->end[1].sink = &relay->dir[ODIN_RELAY_V2_DIR_A];
...
void odin_relay_v2_destroy(odin_relay_v2_t *relay) {
  ...
  free(relay->dir[ODIN_RELAY_V2_DIR_A].buf);
  free(relay->dir[ODIN_RELAY_V2_DIR_B].buf);
  free(relay);
}
