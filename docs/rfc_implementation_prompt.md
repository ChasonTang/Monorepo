...
- **Don't shrink scope.** Every `G#` must be implemented; every `T#` must transition red→green across §8 phases; every §5 migration ships in the phase that introduces the break; every §6 mitigation is implemented no later than the code it protects.
- **Phase order is binding.** Complete §8 phases in declared order. A phase is an execution checkpoint, not a separate Code Review diff: do not begin `P{#+1}` until the current phase's `Done when` clauses have been verified, but do not present or stage the final Code Review change set until every phase is complete.
...
2. Implement only what `Scope` lists. Lift §4.2 contract surfaces into the implementation exactly; lift the §4.2 Mechanism paragraph into runnable code.
...
5. Verify each `Done when` clause against the artifact it cites: grep the §7 row, run the local test suite, read the header doc-comment, inspect the schema change artifact. Do not declare the phase done until every clause holds.
...
2. Verify the final working tree still satisfies every `G#`, every §4.2 contract surface, every §5 migration, every §6 mitigation, and every §4.3 ruled-out alternative.
...
3. **RFC coverage:** every `G#`, §4.2 contract surface, §5 migration, §6 mitigation, and §4.3 ruling verified against the staged diff.
4. **Deviations from RFC:** list any, each with — where the RFC said X, what you did instead, why (cite the user's clarifying answer; never invent justification). `None` is the expected default.
