...
- **Don't shrink scope.** Every `G#` must be implemented; every `T#` must transition red→green across §7 phases; every §4 migration ships in the phase that introduces the break; every §5 security mitigation is implemented no later than the code it protects.
...
2. Verify the final working tree still satisfies every `G#`, every §3.2 contract surface, every §4 migration, every §5 security mitigation, and every §3.3 ruled-out alternative.
...
3. **RFC coverage:** every `G#`, §3.2 contract surface, §4 migration, §5 security mitigation, and §3.3 ruling verified against the staged diff.
4. **Deviations from RFC:** list any, each with — where the RFC said X, what you did instead, why (cite the user's clarifying answer; never invent justification). `None` is the expected default.
