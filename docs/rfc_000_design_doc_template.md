...
- **Single-RFC hard caps.** The final RFC body, after deleting template instructions and excluding `## Revision Notes`, must stay at or below 750 lines. §5 must stay at or below 20 `T#` rows. Each `T#` table row must stay at or below 2500 characters and at or below 8 named subcases. §3.2 must stay at or below 5 detailed-design subsections. A single RFC must not cross more than 5 independent module or ownership boundaries. If the requirement would exceed any cap, stop and split the requirement before drafting; do not hide complexity by packing many branches into one oversized `T#` row. Exceptions require explicit user approval and an `Exception:` paragraph before §1 naming the exceeded cap and why the work cannot be split.
